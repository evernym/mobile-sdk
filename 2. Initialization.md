...
- Registration requires the sponsor to provide these attribute to Evernym’s TE department: 
    ...
    `msg-forwarding`: Optional Object which contains the type of transport protocol in which Evernym’s cloud services should forward messages to a Sponsor’s backend that are addressed to a sponsee, thus enabling the sponsor to push notify to a sponsee’s device without sharing push notification keys with Evernym.
     ```json
          {
...
3. Receiving Future Messages
    - ****Somehow the app will need to provide a push notification endpoint. When Evernym's Agency receives a message for this entity (through provisioned cloud agent), the cloud agent will forward the message to the sponsor. 
    ...
    - HTTP Endpoint:  This is found in the Sponsor's configuration. Future messages for the `sponsee` will then be forwarded to that endpoint. The message will include sponsee details (details identifying which sponsee the sponsor should forward to) and the msg id to be downloaded.
...
2. Provisioning
    ...
    5. The sponseeId field in the generated provision token will be the `push token (e.g firebase key, address)`. The `push token` was provided to the tokenizer api as a way to communicate back to the sponsee. The newly generated cloud agent will register this sponseeId as the communication method between itself and the sponsee (fully provisioned Connect.me app).  
