...
Almost all of the iOS APIs provided by the Connect.Me iOS SDK are asynchronous, which means that a completion callback function is required to get the results of the function invocations. This page contains the steps to initialize the wallet and a Verity cloud agent. Most of these wallet initialization steps are done in the AppDelegate’s `didFinishLaunchingWithOptions` lifecycle method. <!--[Q1] What about Android?-->
...
### Definitions
- Sponsor \
   Application owner using the mobile SDK to sponsor individual app installs i.e enabling apps to provision with `Verity ` \
>   **Example** Credit Union is the Sponsor using the Evernym mobile SDK inside their own mobile app.

    
- Sponsee \
    Individual App install.
>   **Example**   Ryan is a Sponsee. He is an Example Credit Union member installing Example CU’s mobile app (`sponsor`) 

- Verity \
    Evernym Agency which facilitates provisioning and management of cloud agents - Evernym tokenizer service, provisioning protocol, and agent management for all other protocols.
...
- Provision Token \
    ...
    ```
    `sponseeId`:
     An identifier a Sponsor uses to identify/reference/communicate with the sponsee. This can be a customer id used to identify the sponsee in the sponsor’s back end database or an endpoint used to directly forward messages.  
    `sponsor`: 
     The name of the entity sponsoring the provisioning. \
    `nonce`:
    ...
    **Example**. random.number.toString() or random.uuid() \
    ...
    `sig`:
    The sponsor signs the timeStamp + the nonce + customerId and encodes it to a base64 encoded string. The Sponsor uses the signing keys associated with the verity registered verkey to perform the signature. 
...
### Sponsor Registration with Verity
- Currently sponsors will need to interact out of band with Evernym to register with Verity. Eventually, it's possible that a protocol may facilitate this interaction. 
- Registration requires the sponsor to provide these attribute to Evernym’s TE department: 
    ...
    `persistent-id`: An ID which will not change. Keys cannot be used because of rotation possibilities. (May be assigned by TE) \
    `keys`: VerKey associated with the signing of the token and used to verify a sponsee’s provision token. 
    - The signing keys (including the verkey shared with Verity) can be generated using this simple tool: `https://github.com/sovrin-foundation/launch/raw/master/sovrin-keygen.zip` 
    ...
    `msg-forwarding`: Optional Object which contains the type of transport protocol in which Verity should deliver messages
     ```json
          {
            "type": "String" // "HTTP" or "Push Notification", 
            "endpoint": "String" // Only used for HTTP,
          }
     ```
    - `type`: "HTTP" and "Push Notification" are currently the only supported types
    - `endpoint`: Endpoint where HTTP message will be sent. Sponsor's backend can deliver to the app however they want. 
        - This is not used if the type is "Push Notification". A "Push Notification" endpoint would be registered later by the sponsee during provisioning (delivered in `token` -> `sponseeId`)
        
### Mobile SDK - 3rd party apps (`Sponsee`)
1. Obtaining Provision Token
    - An app will have to communicate with its `sponsor` to receive a `token`. The sponsee will need to provide the push notification endpoint of itself in the request so that the generated token will include this. 
    ...
    - It also contains a timestamp. If the token isn’t delivered to Verity in a predefined time frame, the token will be invalid. The `sponsee` will need to request another `token` from its `sponsor`.
...
2. Provisioning
    ...
    - The configuration returned from this api will be the input for `vcx_init`. At this point, the app will be provisioned on verity with a cloud agent and will have a local wallet initialized but NOT open. `Vcx_init` will open the wallet.

3. Receiving Future Messages
    - ****Somehow the app will need to provide a push notification endpoint. When Verity receives a message for this entity (through provisioned cloud agent), the cloud agent will forward the message to the sponsor. 
    - Verity will forward messages to the Sponsor’s back-end vi http. No A2A encryption will be added. The http message will include:
        1. Push-notification endpoint
        2. Msg id: This is the id that the sponsee will use to actually download the message
...
1. Obtaining Provision Token
   ...
   3.  Authentication: The approach that Verity is taking for authentication is that if a token request comes in, the response will be sent via push notification to only CMe. No other apps will receive this token because of the firebase key.
...
2. Provisioning
    ...
    4. The configuration returned from this api will be what is passed into vcx_init. At this point, the app will be provisioned on verity with a cloud agent and will have a local wallet initialized but NOT open. Vcx_init will open the wallet.
    5. The sponseeId field in the generated provision token will be the `puch_com_method`. The `push_com_method` was provided to tokenizer api as a way to communicate back to the sponsee. The newly generated cloud agent will register this sponseeId as the communication method between itself and the sponsee (fully provisioned Connect.me app). 
    
3. Receiving Future Messages
    - When Connect.me sent the provision request (with the provision token), the communication method was provided in the sponseeId field of the token. The cloud agent will register this sponseeId as the communication method.  
