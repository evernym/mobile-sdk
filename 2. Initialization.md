...
- Provision Token \
    ...
    **Example**. `random.number.toString()` or `UUID.randomUUID().toString()` 
    * `timestamp`:
    An RFC 3339 and ISO 8601 date and time string such as `1996-12-19T16:39:57-08:00`.
    * `sig`:
    The `Sponsor` signs the timeStamp + the nonce + customerId + sponsorId and encodes it to a base64 encoded string. The `Sponsor` uses the signing keys associated with the `Evernym Cloud Service` registered verkey to perform the signature. \
    **Example** `Base64Encode(Sign(nonce + timestamp + id + sponsorId))`
    * `sponsorVerKey`: `Sponsor's` verkey associated with the signing keys. `Sponsor` can register multiple keys so this helps identify which key to use. This key is not used unless a corresponding one is foudn in the configuration.
