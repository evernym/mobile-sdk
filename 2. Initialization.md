...
Android API are mostly asynchronous. It uses Future API, namely [android-retrofuture](https://github.com/retrostreams/android-retrofuture) implementation. Most of the methods returns `CompletableFuture` objects, that could be used both synchronous and async ways.
...
2. Save the pool-transactions genesis configuration JSON to a file. The contents of this file will be similar to the contents shown [here](https://raw.githubusercontent.com/sovrin-foundation/sovrin/stable/sovrin/pool_transactions_sandbox_genesis)<!-- TODO: contents outdated, need new source-->. The path to the pool-transactions genesis file is used in the function call to initialize the SDK.

3. On first init (if you don't have populated configuration JSON yet)

    a. Create wallet key and store it in file.

    b. Prepare agency configuration JSON using created wallet and pool data. <!-- TODO add sample of config>

    c. Call `UtilsApi.vcxAgentProvisionAsync(agencyConfig)` with prepared config to fill necessary fields

    d. Add additional fields to resulting JSON (see `ConnectMeVcx#populateConfig`) and store resulting JSON string 
    

4. Call `VcxApi#initSovToken` to initialize libVcx. In case return code is not `0` , you should call  `VcxApi.vcxInitWithConfig(config)` with config created on step _#3.e_
