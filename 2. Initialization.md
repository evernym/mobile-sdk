...
- Provision Token \
    ...
    ```
    `sponseeId`:
     An identifier a Sponsor uses to identify the sponsee requesting to provision. \
    `sponsor`: 
...
- Registration requires the sponsor to provide these attribute to Evernym’s TE department: 
    ...
    `msg-forwarding`: Object which contains the type of transport protocol in which Evernym’s cloud services should forward messages to a Sponsor’s backend that are addressed to a sponsee, thus enabling the sponsor to push notify to a sponsee’s device without sharing push notification keys with Evernym.
     ...
     ```
    - type: "HTTP" is currently the only supported type
...
3. Receiving Future Messages
    - ****Either the push endpoint can be provided in the provision or `update_com_method` can happen after provisioning. 
