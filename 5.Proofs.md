...
To successfully fill and sign a proof request, the following process must be completed: 
  ...
  - [1. Retrieve pending messages](#1-retrieve)
...
<!--## Proof Request Flow
![Proof Request Flow](wiki-images/ProofRequestFlowConnectMe.png) This illustration has a decision point but it doesn't use the diamond. -->
...
String data = DisclosedProofApi.proofRetrieveCredentials(proofHandle).get();
```
...
After sending proof, message update should be sent with `Answered` status.
See messages documentation.
...
After rejecting proof, message update should be sent with `Answered` status.
See messages documentation.
...
Await credential status change. Call following code in loop until returned state is not equal `9` (`Rejected`):
