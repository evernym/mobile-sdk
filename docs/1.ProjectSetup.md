...
* **Currently recommended versions**
  * iOS:
      * `vcx 0.0.221` for phones
      * `vcx 0.0.222` for simulators
  * Android:
      * `com.evernym:vcx:0.12.0-3e2dcc3@aar`
...
4. Add `pod 'vcx', '0.0.221'` in Podfile inside `target <ProjectName>`

    * `vcx 0.0.221` should be used to run applications on phones
    * `vcx 0.0.222` should be used to run applications on simulators
...
2. Open the app-level `build.gradle` file and add the following to the dependencies section.
    ...
    ```gradle
    // VCX library
    implementation 'com.evernym:vcx:0.12.0-3e2dcc3@aar'
