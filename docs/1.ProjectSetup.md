...
* **Currently recommended versions**
  * iOS:
      * `vcx 0.0.223` for phones
      * `vcx 0.0.224` for simulators
  * Android:
      * `com.evernym:vcx:0.13.0-778d12c@aar`
...
4. Add `pod 'vcx', '0.0.223'` in Podfile inside `target <ProjectName>`

    * `vcx 0.0.223` should be used to run applications on phones
    * `vcx 0.0.224` should be used to run applications on simulators
...
2. Open the app-level `build.gradle` file and add the following to the dependencies section.
    ...
    ```gradle
    // VCX library
    implementation 'com.evernym:vcx:0.13.0-778d12c@aar'
