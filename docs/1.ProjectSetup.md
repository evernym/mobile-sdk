...
* **Currently recommended versions**
  * iOS:
      * `vcx 0.0.211` for phones
      * `vcx 0.0.212` for simulators
  * Android:
      * `com.evernym:vcx:0.12.0-b5d64b8@aar`
...
4. Add `pod 'vcx', '0.0.211'` in Podfile inside `target <ProjectName>`

    * `vcx 0.0.211` should be used to run applications on phones
    * `vcx 0.0.212` should be used to run applications on simulators
