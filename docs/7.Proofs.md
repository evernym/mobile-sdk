...
It may happen that User does not have a credential which can be used for filling of requested attribute.\
Sometimes Verifier may also accept proofs where values for missing attributes are filled by the User.

Attributes can be self-attested if one of the conditions met:
* there is not any `restrictions` set for the attribute in the Proof Request
* `self_attest_allowed` is set as `true` in the Proof Request
...
> **NOTE:** Proof can contain only part of used credential data (e.g. only one of the multiple credential fields was revealed in the proof).

> **NOTE:** Proof can build using multiple different credentials (e.g. different credentials were used for requested attributes/predicates).
...
* `~thread.thid` or else `@id` - Identifier of present proof protocol.  (`threadId` field of  application Proof object).
...
1. Retrieve credentials which can be used to fill each requested attribute/predicate in the Proof Request
   ...
   #### Result Structure
    ...
    {
         "attrs": [
            "attribute_or_predicate_referent_in_proof_request": [ // unique identifier of requested attribute/predicate in the proof request
               {
                  // list of credentials which can be used for filling of attribute/predicate referenced by `attribute_or_predicate_referent_in_proof_request`
                  "cred_info": {
   ...
   #### Result Example
    ...
    {
        "attrs": {
            "attribute_1_unique_referent": [ // identifier of first requested attribute in the proof request 
                // only one credential which can be used for filling of attribute referenced by `attribute_1_unique_referent`
                {
            ...
            "attribute_2_unique_referent": [ // identifier of second requested attribute in the proof request 
            ...
            "predicate_1_unique_referent": [ // identifier of first requested predicate in the proof request 
   ...
   #### Important Notes
    ...
    > Credentials for attributes and predicates are located inside the same `attrs` field. 

    > There can be several credentials that can be used for filling of requested attribute/predicate.

    > In case any of the fields contain an empty array, that means that there are no credentials are available for filling of attribute/predicate, and the acceptance process cannot proceed further (Proof Request can be only rejected). 
...
1. Select credentials which will be used proving of each requested attribute / predicate. 
   Basically, you need to select single `credential info` entry for each requested attribute / predicate. 
    ...
    ```
    {
        "attrs": {
            "attribute_1_unique_referent": {  // identifier of first requested attribute in the proof request 
                "credential": {
                    "cred_info": { // selected credential info
                        "attrs": {
                            "Number": "12345",
                            "First Name": "Alice",
                            "Last Name": "Andersen",
