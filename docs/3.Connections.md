...
* Inviter prepares `Connection Invitation`
* Invitee sends `Connection Request` to Inviter
* Inviter handles message and sends `Connection Response` to Invitee
* Invitee handles message and sends `Connection Ack` (if requested) to Inviter
...
Once we get the Сonnection completed we need to store it for future usages.
