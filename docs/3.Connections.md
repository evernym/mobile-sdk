...
2. Accept connection and create Pairwise Cloud Agent:
...
6. It will also be useful to fetch `pw_did` field from the serialized connection object. This field will be needed during the handling of messages received from the Cloud Agent (in order to associate messages with serialized connections).
