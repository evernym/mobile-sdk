...
In order to check does connection already exists, we need to retrieve invitations that were used during the creation of existing connections and compare them with the new ones.
...
1. Compare invitations. Invitations will match when one of the fields equal to earth other:
...
1. If you find that connection already exists for the received invitation you likely better reuse the existing connection rather than establish a new one.
You can read how to do it in the [Connection Redirection document](./7.ConnectionRedirection.md)
...
The connection will be completed when `Connection Response` message received back from the opposite side.
