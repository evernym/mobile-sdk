...
3. On first init (if you don't have populated JSON config) following steps should be performed:
    ...
    4. Call Sponsor server backend API to get provision token. See [MainActivity#retrieveToken()](/examples/android/CMeSdkJava/app/src/main/java/me/connect/sdk/java/sample/MainActivity.java#119)
...
4. On second and other application initializations you must fetch config (generated on the step 3 during the first application launch) from the storage and directly initialize libVcx (see [ConnectMeVcx#initialize()](/examples/android/CMeSdkJava/lib/src/main/java/me/connect/sdk/java/ConnectMeVcx.java#L248).
