...
1. Pool genesis transaction using for Ledger connection can be found [here](/examples/android/CMeSdkJava/app/src/main/res/raw/genesis.txt).
   ...
   For sample, [writeGenesisFile()](/examples/android/CMeSdkJava/lib/src/main/java/me/connect/sdk/java/Utils.java).

1. Selected Agency and other application configuration settings can be found [here](/examples/android/CMeSdkJava/app/src/main/java/me/connect/sdk/java/sample/Constants.java).

1. On first init (if you don't have populated JSON config) following steps should be performed:

    1. Configure application file system permissions. See [configureStoragePermissions](/examples/android/CMeSdkJava/lib/src/main/java/me/connect/sdk/java/Utils.java) function.  
    
    1. Configure application logger. See[configureLogger](/examples/android/CMeSdkJava/lib/src/main/java/me/connect/sdk/java/Logger.java) function.

    2. Generate wallet key. See [createWalletKey()](/examples/android/CMeSdkJava/lib/src/main/java/me/connect/sdk/java/Utils.java) function.

    3. Prepare configuration JSON (see [config sample](#sdk-provisioning-config-sample-single-pool-ledger)).

    4. Call Sponsor server backend API to get provision token. See [retrieveToken()](/examples/android/CMeSdkJava/lib/src/main/java/me/connect/sdk/java/Initialization.java)
