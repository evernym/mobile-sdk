...
Simple server performing provision token signing and provision token generation could be found [here](/examples/simple-sponsor).
...
1. On **first** application init:
    ...
    * Call VCX function to provision an Agent with received token
...
```json
{
  ...
  "agent_seed": null,  // Agent seed
  "enterprise_seed": null, // Enterprise seed
