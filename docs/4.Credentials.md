...
* Holder sends `Credential Request` message to Issuer
* Issuer handles message and sends `Credential` message to Holder
* Holder handles message and sends `Credential Ack` message (if requested) to Issuer
...
1. Await Credential received.
    ...
    On the Holder side the credential-issuance will be completed when `Credential` message received and processed.
    After that, Credential state machine will be moved to the state `4` (`Accepted`). 
