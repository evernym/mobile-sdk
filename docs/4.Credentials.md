...
See [messages documentation](8.Messages.md) for messages downloading information. Pending messages with `credential offer` type should be used.
...
1. Store serialized Credential for latter operations. \
   It is up to the developer regarding what data to store in the application. On of the possible formats may match the following structure:
    ...
    ```
    {
        "thid" - string // thread id (`thread_id` field)
...
1. Every time in the future you want to perform some operations using the created credential object you need firstly to fetch Credential object from the storage and next deserialize SDK object from its serialized representation (receive a new handle).

1. Update status of correspondent message on the Agent. See [messages documentation](8.Messages.md) for message update information.

### 3. Accept Credential Offer

In case user agree to receive provided credentials from the connection, following steps should be performed to receive actual credentials.
...
1. Await credential state is completed.
...
1. Update a related record in the storage with the latest value of the Credential state object.
