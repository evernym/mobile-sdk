...
1. Download pending messages (see [messages documentation](8.Messages.md) for messages downloading information). Pending messages with `credential-offer` type should be used.  Extract credential offer JSON string from the downloaded message (value of `decryptedPayload` field).
...
1. Await Credential is received.
