...
1. Download pending messages (see [messages documentation](8.Messages.md) for messages downloading information). Pending messages with `proof request` type should be used.  Extract Proof Request JSON string from the downloaded message (value of `decryptedPayload` field).
