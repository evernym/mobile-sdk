...
### 1. Retrieve pending messages
See [messages documentation](8.\ Messages.md)

### 2. Parse pending message

From credential offer, following fields could be used:

* `claim_name` - Name of the claim
* `credential_attrs` - JSON object containing pairs of offered credential name and value

For example, see `me.connect.sdk.java.sample.credentials.CredentialOffersViewModel#extractDataFromCredentialsOfferMessage`
...
```
Call until credentail status changes to `4`.
