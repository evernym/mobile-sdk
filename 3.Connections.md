...
3. Await message status change. Call following code in loop until returned state is not equal `4` (`Accepted`). This step is required for `aries` and `out-of-band` protocols:
    ```java
    int state = -1;
    while(state != 4){
...
In certain cases, invitation could be presented in form of URL:

```
https://<domain>/<path>?c_i=<invitationString>
```
`c_i` is required parameter containing Base-64 encoded invitation.
...
In that case, we don't need to reset connection (delete existing connection and create new connection with same pair).

You can find more details about this use case here: [7. Connection redirection](7.ConnectionRedirection.md)

### Error while trying to use previously created connection

After establishing succesfull connection and storing it to your device, once user exists application and return after some time, Vcx library will be reinitialized again. It is expected that, while user tries to use connection created in previously session, using same connection will return error response with a code 1010. In that case, necessary step will be to call this method:

> iOS (ObjC)

```objc
- (void)connectionSendReuse:(VcxHandle)connectionHandle invite:(NSString *)invite withCompletion:(void (^)(NSError *error))completion {
}
```
...
Rest of the flow should continue the same as with in the flow of establishing new connection. 