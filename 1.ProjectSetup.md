...
<!--[Q2] Do they know where to extract the files in their environment? Any special instructions?-->


# Project Setup

<!--![Setup project steps](wiki-images/CredentialsConnectMe.png) I'm not sure we need this illustration: there is no decision tree, only sequential steps, not showing a process that the application runs.-->
...
1. Create a new Xcode project using the Xcode New Project wizard: <!--[Q3] For Android Studio they can use an existing project. Can they use an existing Xcode project?-->
   ...
   * In new window <!--[Q4] "In the new window" or "For **New Window**"?--> select the folder where you would like to store your project and click **Create**. This directory is the *project directory*.
...
2. Edit the Podfile (`mobile-starter-master/ios/CMeSdkObjc/Podfile`) <!--[Q6] Is CMeSdkObjc a directory that the user will have?-->so that it contains the following lines: <!--[Q7} Which line(s) do you mean?-->

<!--[Q8] Should they uncomment the platform line and add a version number? Does it matter which version number?-->
...
```
<!--[Q9] Is the def section something they have to add?-->
...
<!--[Q10] Is the colon in the right place relative to "ios"?-->
...
3. Go to the Evernym repo directory: <!--[Q11] provide URL-->
...
7. You can run a simple HTTP server by installing the `http-server` NPM module: <!--[Q12] When should they set up the HTTP server? Should they do it before editing vcx.podspec?-->
...
8. In the terminal go to the project directory. Run one of the following commands: 
```
pod install
```
or
...
- `Pod install` will hit 0.0.101 <!--[Q13] What's the use case for this? What does it mean to "hit" that number?-->
- `CONNECTME_DEBUG=true pod install` will hit 0.0.102 <!--[Q14] What's the use case for this?-->

10. Verify that `<project_name>.xcworkspace` exists in the project directory. Use *only* that file to open your project and edit your source code in Xcode. *Do not* use the `<project_name>.xcodeproj` file to open or edit your project, but don’t delete it, either.   

10. If you are using Swift, you must create an `Objective-C` bridging header file named `<project_name>-Bridging-Header.h` in the same directory as your `AppDelegate.swift` file. Add it to your Xcode project using the **File > New > File** menu item of the Xcode IDE: 
...
12. Edit `<project_name>-Bridging-Header.h` in the Xcode IDE and make sure it has the following lines: 

<!--[Q15] Do we need this line? > in mobile-starter-master/ios/CMeSdkSwift/CMeSdkSwift/CMeSdkSwift-Bridging-Header.h-->
...
#endif /* <project_name>_Bridging_Header_h */
```

13. If you are using `Objective-C`, edit `AppDelegate.h` to add the following to the `imports` section of the file:

<!--[Q16] Do we need this line? > in mobile-starter-master/ios/CMeSdkObjc/CMeSdkObjc/AppDelegate.h-->
...
<!--- [Q20] Will this section be included for the beta? Who will write it? TODO: Initialize self.sdkApi and self.sdkInited for android?...  mobile-starter-master/android/CMeSdkJava/app/src/main/java/me/connect/sdk/java/MainActivity.java-->
