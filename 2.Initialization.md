...
1. Create a sponsor service/endpoint on your app's backend. A [sample](https://github.com/evernym/mobile-sdk/tree/master/simple-sponsor)
2. Ask Evernym to onboard you as a Sponsor. Check below for more details
3. Complete mobile SDK [setup guide](./1.ProjectSetup.md) first
4. From your mobile app call your backend API (service setup in Step 1) to get provision token
5. Call Evernym mobile SDK provision API using provision token
6. Call Evernym mobile SDK init API
...
1. Get Provision Token
    ...
    - The `sig` field in [Token Fields](2.%20Initialization.md#provision-token-fields) describes how to sign a token.
...
2. Provisioning
    ...
    - Once your app has provision token, then you would need to call `UtilsApi.vcxGetProvisionToken`
    - This API expects a single parameter of string type. This string is JSON stringified Object which has following format
        ...
        ```json
        {
            ...
            source_id: "<any identifier for this request>",
            com_method: { type: 4, id: "<could be same as sponsee_id>", value: "<push token that can be used to send push notification to your customer by you (Sponsor)>" },
            sponsee_id: "<same as inside provision token>",
            sponsor_id: "<your sponsor name that you sent in onboard email>",
        }
        ```

    - The value returned from this api will be the input for `vcx_init`. At this point, the app will be provisioned on Evernym's Cloud Service with a cloud agent and will have a local wallet initialized but NOT open. `Vcx_init` will open the wallet.
...
3. Receiving Your customer's Future Messages
    ...
    - You (`Sponsor`) will then send push notification to the customer (`Sponsee`) informing that a message is available and ready to be downloaded from the customer's (`Sponsee's`) cloud agent.
...
### Example API call to download new messages in Objective-C
