...
+(void)initVCX {
    ...
    [self retreiveProvisioningToken:^(NSString *successMessage, NSError *error) {
        if(error != nil && error > 0) {
            NSLog(@"Provisioning token error: %@", error);

            NSLog(@"Failing back to Init without Provisioning Token");
            [self initializeMobileSDK: agencyConfig withProvisioningToken: token];
            return;
        }
...
+(void)retreiveProvisioningToken: (ResponseBlock) completionBlock {
    if([sponsorServerURL isEqual: @"placeholder"]) {
        NSError *error = [[NSError alloc] initWithDomain: @"connectMe" code: 400 userInfo: @{ @"message": @"Error: Sponsor Server URL is not set."}];
    ...
    [CMUtilities sendPostRequest: sponsorServerURL withBody:@{@"sponseeId": sponseeID, @"pushToken": [LocalStorage getValueForKey:@"pushToken"] } andCompletion:^(NSDictionary *responseObject, NSError *error) {
        ...
        if(responseObject[@"token"] != nil) {
            ...
            [LocalStorage setValue:token forKey:@"provisioningToken"];
